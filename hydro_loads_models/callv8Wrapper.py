...
if __name__ == "__main__":
    ...
    example = 1           # Choose which example to run: 1=pyWrapper, 2=omdaoSingle, 3=omdaoGroup
    # If example = 3, be sure to run callv8Wrapper using RunParallel script file
    # or using equivalent mpi commands, otherwise cases will run in serial.

    record = True    # Specify whether or not we want outputs recorded to sqlite database file

    if example == 1:
        a, b = pyWrapper()
        with open("out2.dat", "w") as out:
