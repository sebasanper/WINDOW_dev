...
import workflow_cheap as wk_irreg
import workflow_cheap_regular as wk_reg
import constraints_openmdao as constraints
import farm_description as fd
import turbine_description as td
import random
import transform_quadrilateral as trans
import numpy as np

def regular():

    prob = Problem()
    prob.model = wk_reg.LCOE()
    prob.setup()

    def obj(x):
        print x  # For stopping the optimiser before convergence.
    ...
    x1 = [random.uniform(800.0, 2500.0), random.uniform(800.0, 2500.0), random.uniform(0.0, 2500.0), random.uniform(0.0, 180.0)]
...
def irregular():
    ...
    prob = Problem()
    prob.model = wk_irreg.LCOE()
    prob.setup()

    class constraint1(Group):    
        def setup(self):
            indep3 = self.add_subsystem('indep3', IndepVarComp())
            indep3.add_output("layout", val=np.array([create_random() for _ in range(fd.NT)]))
            indep3.add_output("radius", val=td.rotor_radius)
            self.add_subsystem("mindistance", constraints.MinDistance())
    ...
    class constraint2(Group):
        def setup(self):
            ...
            indep4.add_output("layout", val=np.array([create_random() for _ in range(fd.NT)]))
            indep4.add_output("areas", val=fd.areas)
            self.add_subsystem("inbounds", constraints.WithinBoundaries())
    ...
    def obj(x):
        print x
        prob['indep2.layout'] = [[x[i], x[i+1]] for i in range(0, fd.NT * 2, 2)]
        prob.run_model()
        ans = prob['analysis.lcoe'][0]
        return ans

    def con1(x):
        cons1['indep3.layout'] = [[x[i], x[i+1]] for i in range(0, fd.NT * 2, 2)]
        cons1.run_model()
        ans = cons1['mindistance.magnitude_violations'][0]

    def con2(x):
        cons2['indep4.layout'] = [[x[i], x[i+1]] for i in range(0, fd.NT * 2, 2)]
        cons2.run_model()
    ...
    opts = {'disp':False, 'maxiter':10000, 'rhobeg':300.0}  #rhobeg is the initial step size.
