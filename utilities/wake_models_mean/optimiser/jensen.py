...
from utilities import wake_models_mean as wake
from utilities.wake_models_mean import Ct, power
from utilities.wake_models_mean import distance_to_front
...
def jensen_angle(layout_x, layout_y, wind_speed, angle):
    ...
    if len(layout_x) == len(layout_y):
        ...
        raise ValueError("layout_x has different length than layout_y")
    ...
    with open('power3.dat', 'w') as out:
        for i in range(40, 900, 10):
            for j in range(900, 1120, 10):
                out.write('%d\t%d\t%f\n' % (i, j, jensen_angle([0., float(i), float(j)], [0.0, 0.0, 0.0], 8.5, 180.0)))
