...
def rainflow(array_ext, flm=0, l_ult=1e16, uc_mult=0.5):
    ...
    print array_out
    answer = [[0.0 for _ in range(len(array_out[0]))] for _ in range(2)]

    for i in range(len(array_out[0])):

        if array_out[0][i] not in answer[0]:
            answer[0][i] = array_out[0][i]
            answer[1][i] = array_out[3][i]
        else:
            place = answer[0].index(array_out[0][i])
            answer[1][place] += array_out[3][i]

    answer[0] = filter(lambda a: a != 0.0, answer[0])
    answer[1] = filter(lambda a: a != 0.0, answer[1])
    answer[1] = [x for (y, x) in sorted(zip(answer[0], answer[1]))]
    answer[0].sort()
    return answer
