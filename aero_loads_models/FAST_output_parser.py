...
def load_ascii_output(filename):
    with open(filename) as f:
        ...
        try:
            ...
            out_dict = {}
            for j in range(len(info["attribute_names"])):
                name = info["attribute_names"][j]
                out_dict[name] = []
                for i in range(len(data)):
                    out_dict[name].append(float(data[i][j]))
...
def load_binary_output(filename):
    ...
    def fread(fid, n, type):
        fmt, nbytes = \
        {'uint8': ('B', 1), 'int16': ('h', 2), 'int32': ('i', 4), 'float32': ('f', 4), 'float64': ('d', 8)}[type]
...
if __name__ == '__main__':
    out, infor = load_output("FAST_run/Test19.out")
