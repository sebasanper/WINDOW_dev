from turbine_description import solidity_rotor, cd_rotor_idle_vane, cd_nacelle, front_area_nacelle, max_thrust, yaw_to_hub_height, mass, mass_eccentricity, yaw_diameter, rotor_radius, wind_speed_at_max_thrust, purchase_price, warranty_percentage
...
class RNA:
    ...
    cd_nacelle = cd_nacelle  # [-] OWTES V66: 1.3, but using a frontal area of 13 m^2
    front_area_nacelle = front_area_nacelle  # [m^2] Vestas V80 brochure: height for transport 4 m, width 3.4 m, rounded up 14 m^2 to include height including cooler top 5.4 m
    max_thrust = max_thrust  # [N] Maximum thrust determined from thrust coefficient curve multiplied with 1.5 amplification factor (determined by Otto for NREL 5 MW turbine)
    yaw_to_hub_height = yaw_to_hub_height  # [m] Vestas V80 brochure: height for transport 4 m - On picture, the axis appears to be in the middle of the nacelle.
    mass = mass  # [kg] 79 tonne nacelle + 3x 6.5 tonne blades
    mass_eccentricity = mass_eccentricity  # [m] - in x-direction, so negative when upwind of tower centre - Just a guess - Vestas V80 brochure: Length of nacelle = 10.4 m
