...
class runFAST_v8(object):
    ...
    def setFastFile(self, fname):
        """ set the name of the FAST (.fst) input file """
        self.fst_file = fname
    ...
    def getOutputValues(self, paramnames, directory=None, out=None, hdr=None):
        ...
        # out contains header info. find our guy, then give back the whole column
        all_out = []
        foundme = [False for _ in range(len(paramnames))]
    ...
    def readED(self):
        ...
        for i in range(len(self.lines_ed)):
            ...
            if len(ln) > 1:
                if ln[1] == "TwrFile" and self.twr_file is None:
                    self.twr_file = ln[0][1:-1]
                elif ln[1] == "BldFile(1)" and self.blade1_fileED is None:
                    self.blade1_fileED = ln[0][1:-1]
                elif ln[1] == "BldFile(2)" and self.blade2_fileED is None:
                    self.blade2_fileED = ln[0][1:-1]
                elif ln[1] == "BldFile(3)" and self.blade3_fileED is None:
    ...
    def readAD(self):
        ...
        for i in range(len(self.lines_ad)):
            ...
            if len(ln) > 1:
                if ln[1] == "ADBlFile(1)" and self.blade1_fileAD is None:
                    self.blade1_fileAD = ln[0][1:-1]
                if ln[1] == "ADBlFile(2)" and self.blade2_fileAD is None:
                    self.blade2_fileAD = ln[0][1:-1]
                if ln[1] == "ADBlFile(3)" and self.blade3_fileAD is None:
                    self.blade3_fileAD = ln[0][1:-1]
                if ln[1] == "NumAFfiles":
                    self.nSeg = int(ln[0])
                    numaf_ln = ln
                    numaf_i = i
                    # if (ln[1] == "WindFile" and self.wind_file is None):  #Wind file now referenced in fst file
        ...
        self.af_dict = {'polar_idx': [0] * self.nSeg, 'polar_files': [0] * self.nSeg}
    ...
    def readBD(self):
        ...
        for i in range(len(self.lines_bd1)):
            ...
            if len(ln) > 1:
                if ln[1] == "BldFile" and self.blade1_fileBD is None:
    ...
    def readFST(self):
        ...
        for line in self.lines_fast:
            ...
            if len(f) < 2:
                continue
            if f[1] == 'BDBldFile(1)' and self.bd_file1 is None:
                self.bd_file1 = f[0][1:-1]
            if f[1] == 'BDBldFile(2)' and self.bd_file2 is None:
                self.bd_file2 = f[0][1:-1]
            if f[1] == 'BDBldFile(3)' and self.bd_file3 is None:
                self.bd_file3 = f[0][1:-1]
            if f[1] == 'EDFile' and self.ed_file is None:
                self.ed_file = f[0][1:-1]
            if f[1] == 'AeroFile' and self.ad_file is None:
                self.ad_file = f[0][1:-1]
            if f[1] == 'ServoFile' and self.sd_file is None:
                self.sd_file = f[0][1:-1]
            if f[1] == 'InflowFile' and self.wind_file is None:
    ...
    def copyServoDir(self):
        ...
        if os.path.exists(runServo):
            ...
            # directory does not yet exist--copy it
    ...
    def writeNoise(self):
        ...
        if self.noise_file is None or self.noise_file == "":
            return
    ...
    def copyAdams(self):
        ...
        # this is executing during write_input, so curdir is run_dir
        if self.adams_file is not None:
    ...
    def writeFST(self, ofname, fstDict):
        ...
        for iln in range(len(self.lines_fast)):
            ...
            flds = line.strip().split()
            if (len(flds) > 1 and flds[1] in ['BDBldFile(1)', 'BDBldFile(2)', 'BDBldFile(3)', 'EDFile', 'AeroFile',
                                              'ServoFile', 'InflowFile']):
                # substituting possibly modified file names for subfiles
...
# ---------------------------------
# Examples unedited for FAST_v8 wrapper, although the general workflow should be consistent
# (these may still work with new executables)
# See "call_Wrapperv8" for examples of how to call code with and without OpenMDAO

def example():
    """ A simple example of running FAST one time """
    fast = runFAST_v8()
    fast.fst_exe = "/home/sebasanper/Downloads/FAST_v8.15.00a-bjj/bin/FAST_glin64"
    ...
    fast.run_dir = "FAST_run"  # either abs or rel path ok

    # all changes to FAST input params go through dict
    # unless you want to change the name of the template for the sub-files (e.g. subst a different aerodyn file).
    # Those go through actual fields ("fast.ad_file")

    fast.fstDict['Vhub'] = 8.0
    fast.fstDict['RotSpeed'] = 12.03
    fast.fstDict['TMax'] = 20.0
    ...
    fast.execute()
    time = list(fast.getOutputValue("Time"))
...
def get_options():
    ...
    options1, args1 = parser.parse_args()
